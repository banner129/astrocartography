...
import { getMidheavenCalculatorPage } from "@/services/page";
import MidheavenCalculatorClient from "./midheaven-calculator-client";
...
export async function generateMetadata({
  ...
  const page = await getMidheavenCalculatorPage(locale);
  const title = page.metadata.title;
  const description = page.metadata.description;
  const keywords = page.metadata.keywords || "";
  const path = "/midheaven-calculator";
...
type PageWithTool = Awaited<ReturnType<typeof getMidheavenCalculatorPage>> & {
  tool: {
...
export default async function MidheavenCalculatorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await params;
  const page = (await getMidheavenCalculatorPage(locale)) as PageWithTool;
  ...
  if (!page.tool) {
    throw new Error("Midheaven calculator page is missing tool labels. Check midheaven-calculator/en.json.");
  }
  ...
  return (
    <>
      ...
      <div className="py-6 lg:py-10">
        <MidheavenCalculatorClient tool={page.tool} />
      </div>
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "WebApplication",
            name: "Midheaven Calculator",
            description: page.metadata.description,
            url: getCanonicalUrl(locale, "/midheaven-calculator"),
            ...
            featureList: [
              "Midheaven (MC) sign and degree by birth time and location",
              "IC opposite and modern sign ruler",
              "Midheaven by city (geographic perspective)",
              "One-click handoff to Astrocartography map for Sun & Jupiter MC lines",
            ],
            creator: {
              "@type": "Organization",
          ...
          __html: JSON.stringify({
            ...
            itemListElement: [
              { "@type": "ListItem", position: 1, name: "Home", item: getCanonicalUrl(locale, "/") },
              {
                ...
                position: 2,
                name: "Midheaven Calculator",
                item: getCanonicalUrl(locale, "/midheaven-calculator"),
              },
            ],
          }),
        }}
