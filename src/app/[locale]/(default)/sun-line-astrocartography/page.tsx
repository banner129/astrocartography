...
import { getSunLinePage } from "@/services/page";
...
const PATH = "/sun-line-astrocartography";
const LOCALES = ["en"];
...
export async function generateMetadata({
  ...
  const page = await getSunLinePage(locale);
...
export default async function SunLinePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              { "@type": "ListItem", position: 3, name: "Sun Line", item: getCanonicalUrl(locale, PATH) },
            ],
          }),
        }}
      />
