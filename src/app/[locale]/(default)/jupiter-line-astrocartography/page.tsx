...
import { getJupiterLinePage } from "@/services/page";
...
const PATH = "/jupiter-line-astrocartography";
const LOCALES = ["en"];
...
export async function generateMetadata({
  ...
  const page = await getJupiterLinePage(locale);
...
export default async function JupiterLinePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              { "@type": "ListItem", position: 3, name: "Jupiter Line", item: getCanonicalUrl(locale, PATH) },
            ],
          }),
        }}
      />
