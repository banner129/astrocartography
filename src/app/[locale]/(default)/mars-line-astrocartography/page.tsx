...
import { getMarsLinePage } from "@/services/page";
...
const PATH = "/mars-line-astrocartography";
const LOCALES = ["en"];
...
export async function generateMetadata({
  ...
  const page = await getMarsLinePage(locale);
...
export default async function MarsLinePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              { "@type": "ListItem", position: 3, name: "Mars Line", item: getCanonicalUrl(locale, PATH) },
            ],
          }),
        }}
      />
