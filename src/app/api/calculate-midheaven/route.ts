...
export async function POST(request: NextRequest) {
  try {
    const body = (await request.json()) as MidheavenRequest;
    ...
    const signIndex = signIndexFromLongitude(mcLon);
    const sign = SIGNS[signIndex];
    const degree = degreeInSign(mcLon);
    const ruler = SIGN_RULERS[signIndex] ?? "—";

    const icLon = normalizeDegrees(mcLon + 180);
    const icSignIndex = signIndexFromLongitude(icLon);

    const otherCities = GEO_CITIES.map((city) => {
      const cityMcLon = calculateMidheavenLongitude(utcTime, city.lng);
      const citySignIndex = signIndexFromLongitude(cityMcLon);
      const cityDegree = degreeInSign(cityMcLon);
      return {
        cityName: city.name,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        ic: {
          sign: SIGNS[icSignIndex],
          degree: Math.round(degreeInSign(icLon) * 100) / 100,
          longitude: icLon,
        },
        otherCities,
      },
